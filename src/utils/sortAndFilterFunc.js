...
const filterSectionFunc = (state, productData) => {
        return state.filterSectionVal.length === 0? productData : [ ...productData ].filter(item => item.isInSection.some(section => state.filterSectionVal.includes(section)))
}

const filterCategoryFunc = (state, productData) => {
        return state.filterCategoryVal.length === 0? productData : [ ...productData ].filter(item => state.filterCategoryVal.includes(item.categoryName))
}

const filterProductsFunc = (state, productData) => {
        return state.filterProductsVal.length === 0? productData : [ ...productData ].filter(item => state.filterProductsVal.some(specialProduct => item[specialProduct] === true))
}

export { sortPriceFunc, sortRatingFunc, sortDiscountFunc, filterPriceFunc, filterSectionFunc, filterCategoryFunc, filterProductsFunc };